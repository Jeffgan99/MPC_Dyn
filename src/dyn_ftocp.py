...
class FTOCP(object):
    ...
    #
    #     # phi_next = 0 + self.dt * (1 / Iz * (lf * Fyf * np.cos(u[0]) - lr * Fyr))
    #     vx_next = x[2] + self.dt * (u[1] - 1 / m * Fyf * np.sin(u[0]))
    #     vy_next = 0 + self.dt * (1 / m * (Fyf * np.cos(u[0]) + Fyr))
    #     ephi_next = x[3] + self.dt * (x[2])
    #     x_next = x[0] + self.dt * (vx_next * np.cos(x[3]) - vy_next * np.sin(x[3]))
    #     y_next = x[1] + self.dt * (x[2] * np.sin(x[3]) + vy_next * np.cos(x[3]))
    #
    #
    #     state_next = [x_next, y_next, ephi_next, vx_next]
    #
    #     return state_next

# def NonLinearBicycleModel(X, u):
#     x = X[0]
#     y = X[1]
#     vx = X[2]
#     yaw = X[3]
...
#     m = 1500.0
#
#     x = x + vx * np.cos(yaw) * dt - vy * np.sin(yaw) * dt
#     y = y + vx * np.sin(yaw) * dt + vy * np.cos(yaw) * dt
#     yaw = yaw + omega * dt
#     Ffy = -Cf * math.atan2(((vy + Lf * omega) / vx - u[0]), 1.0)
#     Fry = -Cr * math.atan2((vy - Lr * omega) / vx, 1.0)
#     R_x = c_rl * vx
#     F_aero = c_a * vx ** 2
#     F_load = F_aero + R_x
#     vx = vx + (u[1] - Ffy * math.sin(u[0]) / m - F_load / m + vy * omega) * dt
#     vy = vy + (Fry / m + Ffy * math.cos(u[0]) / m - vx * omega) * dt
#     omega = omega + (Ffy * Lf * math.cos(u[0]) - Fry * Lr) / Iz * dt
#
#     state_next = [x, y, vx, yaw]
#
#     return state_next
